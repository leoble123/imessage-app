...
impl ImessageCore {
    ...
    /// Moves this account to a different registration relay, keeping the
    /// Apple ID session.
    ///
    /// Which relay serves the validation data is the one variable that has
    /// never been changed while everything else was ruled out, and it is the
    /// variable that decides whether Apple treats this endpoint as real
    /// hardware. Validation data is generated by Apple's own
    /// IMDAppleServices; a relay running on genuine hardware serves the
    /// article, and one running on an emulated machine serves something that
    /// parses, registers, and is answered - with nobody.
    ///
    /// Signing out would do this too, and would also throw away the Apple ID
    /// session and put the whole two-factor gauntlet back in the way. None of
    /// that is necessary: the users and the device identity live in the saved
    /// registration, `bring_up` restores them, and only the config changes.
    ///
    /// The running client has to go first. Its resource manager owns the old
    /// push connection and will reconnect it the moment the new one evicts
    /// it, and then the new one reconnects, and neither survives - the same
    /// eviction loop that made an earlier build unusable. Dropping the client
    /// and the connection before opening the next one is what stops that.
    pub async fn switch_relay(
    ...
    ) -> Result<(), CoreError> {
        {
            ...
            inner.config = None;
            inner.relay_identity = None;
            // The receive loop consumes a broadcast channel belonging to the
            // connection just dropped, so it ends with it; clearing this lets
            // start() spin up a new one rather than deciding it already has.
            inner.receiving = false;
            inner.audio.clear();
        }

        // Anything cached against the old registration is about to be wrong.
        let _ = std::fs::remove_file(self.paths.id_cache());

        self.configure_relay(host, code, token).await?;
        self.force_register().await
    }
